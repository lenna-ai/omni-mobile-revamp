...
import BSSheetMore from "./components/BSSheetChatRoom/BSSheetMore";
import CheckImageLeftChatRoom from "./components/CheckImageLeftChatRoom";

import {Text,View,Image,Linking,Keyboard,Platform,FlatList,TextInput,StyleSheet,ScrollView,Dimensions,TouchableOpacity,ActivityIndicator,KeyboardAvoidingView,ImageBackground, SafeAreaView,} from "react-native";
import { getData, putData, postDataOutHeader } from "./../../modules/services";
import {getRoomId,addDataChats,moreDataChats,pushDataLoading,} from "../../modules/redux/actions/inChat";
import {addDataForAssignRoomId,addDataForAssignLivechat,addDataForAssignAgentInfo,} from "../../modules/redux/actions/member/inForAssign";
...
const ChatRoom = ({ route, navigation }) => {
  ...
  const requestToServer1 = async () => {
    ...
    if (res.data.messages != 0) {
      console.log("data_res_message", res.data);
      let messageFirst = [...dataItem.messages, ...res.data.messages.reverse()];
      // setDataChats(messageFirst);
      dispatch(addDataChats(messageFirst));
      setFirstMsg(res.data.messages[res.data.messages.length - 1].id);
    } else {
      if (firstMessage.messages != null || firstMessage.messages.length != 0) {
        let messageFirst = firstMessage.messages;
        dispatch(addDataChats(messageFirst));
      }
    }
    setIsLoading(false);
    ...
    if (msg.indexOf("/") == 0) {
      ...
      if (data != "") {
        let filteredName = dtMsgTemp.filter((item) => {
          return (
            item.key.toLowerCase().match(data.toLowerCase()) ||
            item.content.toLowerCase().match(data.toLowerCase())
          );
        });
    ...
    if (statusAction == "Resolved") {
      ...
      setTimeout(() => {
        toastFailed.current.show(
          "Cannot Assign agent when status is resolved",
          1000
        );
      }, 500);
    } else {
  ...
  const containMsgTypeLocation = (item) => {
    return (
      <TouchableOpacity onPress={() => openGps(item.latitude, item.longitude)}>
        <CheckImageChatRoom url={item.thumbnailImageUrl} />
        <Text
          style={{
  ...
  const itemContainMsgTypeCarousel = React.useCallback((item, index) => {
    return (
      <View
        style={{
          marginTop: 3,
      ...
      <View>
        ...
        >
          <CheckImageChatRoom
            ...
            size={dimenWidth - moderateScale(120)}
          />
          {/* <ImageElement 
                        style={{
                            resizeMode: 'cover',
    ...
    return (
      ...
      >
        <Image
          source={{
            uri:
              "https://marketplace.canva.com/MAB5sssN0Qw/1/thumbnail_large/canva-file--MAB5sssN0Qw.png",
          }}
  ...
  const leftBlueBubleChat = React.useCallback(
    (item, date, value, valueText, valueFirst) => {
      let valueTopLeft = 0;
      ...
      return (
        <View style={{ alignItems: "flex-start" }}>
          <View
            style={{
              ...styles.bubbleBoxLeft,
              marginVertical: value == 0 && valueText == 0 ? 4 : 0,
            }}
          >
            <View style={styles.offsetBubbleleft} />
            {containMsgType(item)}
          </View>
          <Text
            style={{
  ...
  const rightWhiteBubleChat = React.useCallback(
    (item, date, value, valueText, valueFirst, index) => {
      let valueTopRight = 0;
      ...
      return (
        <View>
          <View
            style={{
              ...styles.bubbleBoxRight,
              marginVertical: value == 0 && valueText == 0 ? 7 : 4,
            }}
          >
            <View style={styles.offsetBubbleRight} />
            {containMsgType(item)}
          </View>
    ...
    return (
      ...
      >
        <View style={{ paddingHorizontal: 10, marginBottom: 20 }}>
          <CheckImageLeftChatRoom
            url={
              item.messageable != null
                ? item.messageable.picture
                : "https://app.lenna.ai/app/public/images/pictures/no_avatar.jpg"
            }
            wd={moderateScale(34)}
            hd={moderateScale(34)}
            value={value}
      ...
      >
        ...
        <View style={{ paddingHorizontal: 10, marginBottom: 20 }}>
          ...
          item.messageable_type == "user_platform"
            ? dataUser.user_id == item.messageable_id
              ? imageIconAdminDefault(
                  moderateScale(34),
                  moderateScale(34),
                  value
  ...
  const renderDataMsg = React.useCallback(({ item, index }) => {
    let valueOpacity = 1;
    let textOpacity = 0;
    let valueFirst = 0;
  ...
  const scrollToDown = () => {
    refDataChats.current.scrollToOffset({ animated: true, offset: 0 });
  };
  ...
  const sendCharts = async (msg, type) => {
    ...
    let dataChatMic = await postDataOutHeader(
      API.PATH + encodeHashIds(dataUser.app_id) + "/chat/send-message",
      setParamsChat(msg, type),
      headers
    );

    if (dataChatMic.success) {
      scrollToDown();
    }
  };
  ...
  const renderItemMsgTemp = React.useCallback(({ item, index }) => {
    if (item.id != 0) {
      return (
  ...
  return (
    <View style={[styles.container]}>
      <ImageBackground
        source={require("../../assets/image/bg-chat.png")}
        resizeMode="stretch"
      ...
      >
            ...
            <View style={styles.container}>
              {dataChats.length != 0 ? (
                <FlatList
                  ...
                  windowSize={dataChats.length * 21}
                  onEndReached={() => dataLoadMore()}
                  showsVerticalScrollIndicator={false}
                  initialNumToRender={dataChats.length}
          ...
          {Platform.OS == "android" ? (
            <View>
              <KeyboardAvoidingView>
                ...
                >
                  <View style={styles.iconAttachment}>
                    <TouchableOpacity
                      onPress={() => refRBSheetAttach.current.open()}
                    >
                      {imageIconAttachment(
                        moderateScale(24),
                        moderateScale(24)
                  ...
                  {chatSend.length ? (
                    <>
                      <View>
                        <TouchableOpacity
                          onPress={() => onSendChat(chatSend, "text")}
                        ...
                        >
                          ...
                          <Text style={styles.labelSend}>Send</Text>
                        </TouchableOpacity>
                      </View>
                    </>
                  ) : (
              ...
              {showMsgTemp == 1 ? (
                <View style={{ ...styles.boxMgsTemp, bottom: heightChatSend }}>
                  <FlatList
              ...
              {showMsgTemp == 1 ? (
                <View
                  style={{
                    ...
                    bottom: showKeyword
                      ? moderateScale(heightChatSend) +
                        moderateScale(keyboardHeight - 16)
                      : moderateScale(heightChatSend) +
        ...
        <BSSheetMore
          ...
          resolvedRooms={resolvedRooms}
          refRBSheetMore={refRBSheetMore}
        />

        <BSAttachSendChat
          refRBSheetAttach={refRBSheetAttach}
          id={id}
